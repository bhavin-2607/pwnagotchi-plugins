## **🔧 Updated Pwnagotchi Plugin (MQTT or HTTP, Configurable Format)**  

Now, you can configure it like this in `/etc/pwnagotchi/config.toml`:  
```toml
...
## **🚀 What’s New?**
✅ **Uses a structured format** like `main.plugins.mac_logger.mqtt.broker`.  
✅ **Easy switching between MQTT and HTTP** with `mode = "mqtt"` or `"http"`.  
✅ **Cleaner configuration and better error handling.**  
...
Now, you can easily switch between **MQTT and HTTP** using the config file without modifying the script! 🚀  
